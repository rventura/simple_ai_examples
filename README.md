...
These examples use code from the Russell & Norvig's "Artificial Intelligence: A Modern Approach" (AIMA) seminal textbook, freely available in the following [official repository](https://github.com/aimacode/aima-python).
...
The folder `uncertainty` contains the following Jupyter notebooks:
- `Bayes.ipynb` -- interactive demo of the Bayes rule
- `Burglary.ipynb` -- shows a few probabilistic inferences on the burglary Bayes network from AIMA

Same instructions as in Search methods above to run any of these examples.
